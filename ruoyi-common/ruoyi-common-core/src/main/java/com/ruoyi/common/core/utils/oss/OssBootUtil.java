package com.ruoyi.common.core.utils.oss;
...
import com.ruoyi.common.core.utils.oss.filter.StrAttackFilter;
...
public class OssBootUtil {
    ...
    private static String accessKeyId = "LTAI5tM2Qj4xyvzEqVzKnkoW";
    private static String accessKeySecret = "IQJwnVy2xiAiPoIbDUWuRflRZPtzvQ";
    //这里推荐，创建RAM用户，创建对应的AccessKey，然后直接复制过来就行

    private static String staticDomain = "dongpinhuidd10.oss-cn-shenzhen.aliyuncs.com";
     ...
     * @return
     */
    private static String bucketName = "dongpinhuidd10";
            ...
            switch (eventType) {
                ...
                case REQUEST_BYTE_TRANSFER_EVENT:
                    ...
                    if (this.totalBytes != -1) {
                        ...
                        percent = (int) (this.bytesWritten * 100.0 / sum);
                        // 将进度percent放入session中
                        session.setAttribute("upload_percent", percent);
                    }
                    break;
...
//            PutObjectResult result = ossClient.putObject(new PutObjectRequest(newBucket, fileUrl.toString(), file.getInputStream()).
...
//            ossClient.setBucketAcl(newBucket, CannedAccessControlList.PublicRead);
    ...
    public static String uploadExt(MultipartFile file, String fileDir, String customBucket,HttpServletRequest request,long size) {
        String filePath = null;
        initOss(endPoint, accessKeyId, accessKeySecret);
        StringBuilder fileUrl = new StringBuilder();
        String newBucket = bucketName;
        ...
        try {
            ...
            // 上传文件的同时指定进度条参数。此处PutObjectProgressListenerDemo为调用类的类名，请在实际使用时替换为相应的类名。
            PutObjectResult result = ossClient.putObject(new PutObjectRequest(newBucket, fileUrl.toString(), file.getInputStream()).
                    <PutObjectRequest>withProgressListener(new PutObjectProgressListener(request.getSession(),size)));
