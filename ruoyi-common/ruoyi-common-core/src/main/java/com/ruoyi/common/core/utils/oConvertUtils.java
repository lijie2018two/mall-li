...
public class oConvertUtils {
    ...
    public static boolean isOverlap(long start1Timestamp, long end1Timestamp, long start2Timestamp, long end2Timestamp) {
        // 如果时间段 1 的开始时间戳在时间段 2 内或者时间段 1 的结束时间戳在时间段 2 内
        if ((start1Timestamp > start2Timestamp && start1Timestamp < end2Timestamp) || (end1Timestamp > start2Timestamp && end1Timestamp < end2Timestamp)) {
            return true;
        }
        // 如果时间段 2 的开始时间戳在时间段 1 内或者时间段 2 的结束时间戳在时间段 1 内
        if ((start2Timestamp > start1Timestamp && start2Timestamp < end1Timestamp) || (end2Timestamp > start1Timestamp && end2Timestamp < end1Timestamp)) {
            return true;
        }
        return false;
    }
     ...
     * 判断字母和字数长度
     */
    public static boolean checkLettersAndDigits(String input, int maxLetters, int maxDigits) {
        int letterCount = input.replaceAll("[^a-zA-Z]", "").length();
        int digitCount = input.replaceAll("[^0-9]", "").length();

        return letterCount <= maxLetters && digitCount <= maxDigits;
    }
