...
@Component
@Slf4j
@ServerEndpoint("/socket/notificationList/{userId}")
public class WebSocketNotificationServer {
    ...
    public static void setCcairbagNotificationMapper(CcairbagNotificationMapper notificationMapper) {
        WebSocketNotificationServer.ccairbagNotificationMapper = notificationMapper;
    }
    ...
    private static  ConcurrentHashMap<String, WebSocketNotificationServer> webMessagetMap = new ConcurrentHashMap<>();
    /**与某个客户端的连接会话，需要通过它来给客户端发送数据*/

    public static ConcurrentHashMap<String, WebSocketNotificationServer> getWebSocketMap() {
...
//        Long userIds = loginAppUser.getCcairbagUsers().getUserId();
        ...
        List<CcairbagNotification> list = ccairbagNotificationMapper.selectNotificationList(Long.valueOf(userId));
//        extracted(list);
        Gson gson = new Gson();
